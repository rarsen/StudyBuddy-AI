...
const SUBJECT_META = {
  mathematics: { label: 'Mathematics', tone: 'from-blue-500 to-indigo-600', chip: 'bg-blue-50 text-blue-700 border-blue-100', emoji: '📐' },
  physics: { label: 'Physics', tone: 'from-purple-500 to-fuchsia-600', chip: 'bg-purple-50 text-purple-700 border-purple-100', emoji: '⚛️' },
  chemistry: { label: 'Chemistry', tone: 'from-emerald-500 to-teal-600', chip: 'bg-emerald-50 text-emerald-700 border-emerald-100', emoji: '🧪' },
  biology: { label: 'Biology', tone: 'from-green-500 to-emerald-600', chip: 'bg-green-50 text-green-700 border-green-100', emoji: '🧬' },
  computer_science: { label: 'Computer Science', tone: 'from-indigo-500 to-violet-600', chip: 'bg-indigo-50 text-indigo-700 border-indigo-100', emoji: '💻' },
  history: { label: 'History', tone: 'from-amber-500 to-orange-600', chip: 'bg-amber-50 text-amber-700 border-amber-100', emoji: '🏛️' },
  literature: { label: 'Literature', tone: 'from-pink-500 to-rose-600', chip: 'bg-pink-50 text-pink-700 border-pink-100', emoji: '📖' },
  language: { label: 'Language', tone: 'from-cyan-500 to-sky-600', chip: 'bg-cyan-50 text-cyan-700 border-cyan-100', emoji: '🗣️' },
  economics: { label: 'Economics', tone: 'from-orange-500 to-amber-600', chip: 'bg-orange-50 text-orange-700 border-orange-100', emoji: '📊' },
  other: { label: 'Other', tone: 'from-slate-500 to-gray-600', chip: 'bg-slate-50 text-slate-700 border-slate-100', emoji: '✨' },
}

const meta = (s) => SUBJECT_META[s] || SUBJECT_META.other
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary-600 via-primary-700 to-accent-700 text-white p-8 sm:p-10 shadow-glow-lg animate-fade-in-up">
          ...
          <div className="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <div>
              ...
              <p className="mt-2 text-white/85 text-lg max-w-xl">
                Pick up a past session or start something new — MindSpark is ready to help.
              </p>
            </div>
          ...
          <div className="relative mt-8 grid grid-cols-3 gap-4 max-w-xl">
            <div className="rounded-2xl bg-white/10 border border-white/15 backdrop-blur-sm p-4">
              ...
              <div className="text-xs text-white/75 mt-0.5">Study sessions</div>
            </div>
            <div className="rounded-2xl bg-white/10 border border-white/15 backdrop-blur-sm p-4">
              <div className="text-2xl font-display font-extrabold">{totalMessages}</div>
              <div className="text-xs text-white/75 mt-0.5">Messages</div>
            </div>
            <div className="rounded-2xl bg-white/10 border border-white/15 backdrop-blur-sm p-4">
              <div className="text-2xl font-display font-extrabold">{subjectsInUse.length}</div>
              <div className="text-xs text-white/75 mt-0.5">Subjects</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-10">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <h2 className="font-display text-2xl font-bold text-ink-900">Your study sessions</h2>
          ...
          {loading ? (
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="card animate-pulse">
                  <div className="h-6 bg-ink-100 rounded w-3/4 mb-4" />
            ...
            <div className="relative overflow-hidden rounded-3xl border-2 border-dashed border-ink-200 bg-white p-12 text-center">
            ...
            <div className="rounded-2xl border border-ink-100 bg-white p-10 text-center text-ink-600">
              No sessions match your filters.
            </div>
          ) : (
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((session) => {
                const m = meta(session.subject)
                return (
                  <Link
                    key={session.id}
                    to={`/chat/${session.id}`}
                    className="group relative overflow-hidden rounded-2xl bg-white border border-ink-100 shadow-soft hover:shadow-glow hover:-translate-y-1 transition-all duration-300"
                  >
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        ...
                        <span className={`chip border ${m.chip}`}>{m.label}</span>
                      </div>

                      <h3 className="font-display text-base font-semibold text-ink-900 line-clamp-2 min-h-[2.75rem] group-hover:text-primary-700 transition-colors">
                        {session.title}
                      </h3>
